...
*FastAPI · SQLite · Python 3.12 · OpenAI-compatible LLM (keyword-heuristic mock by default)*
...
| FP pattern discount accumulates monotonically with entity history | Benchmark Table 5: fp_pattern_score 0.000 → 1.000 as n grows from 0 to 30 events |
| Memory requires ≥ 20 entity events before reliably reducing FPR | Benchmark Table 5: score flips below threshold at n=20, not n=5 or n=10 |
| API accepts any OpenAI-compatible LLM endpoint | Configurable via `LLM_API_KEY` + `LLM_BASE_URL` env vars |
...
| Memory improves overall FPR | Benchmark shows FPR worsens with shallow history (n<18): C0=48.9% → C3=53.3% |
...
**Key formula:** `effective_history = history_score × (1 − fp_pattern_score × 0.80)`
...
- `fp_confidence` — accumulated confidence that this entity is a benign FP source (blended: `new = prior × 0.6 + current × 0.4`)
- `risk_trend` — `stable` / `escalating` / `deescalating`
- `known_good_hours` — hours of day where activity is historically normal
- `dominant_event_types` — alert types that are routine (≥10% of events)
...
Step 5 — Calibration
  ...
  calibrated_score = composite × confidence + 50 × (1 − confidence)
```
...
- Python 3.11 or 3.12
- No API key required — runs in keyword-heuristic mock mode by default
...
GET /memory/by/src-ip?ip=10.0.0.5
GET /memory/by/username?username=admin
GET /memory/by/hostname?hostname=host-01
GET /graph/{entity_id}                     — entity relationship graph
GET /health                                — system status + LLM mode
```
...
After the first analysis `semantic_profile.is_new_entity` is `true`. After the second it is `false` — `fp_confidence` has begun accumulating. Note: reliable FPR reduction requires ≥ 20 entity events (see benchmark Table 5).
...
│       ├── llm_client.py            # OpenAI-compatible client + keyword-heuristic mock
...
| `LLM_API_KEY` | *(empty)* | LLM API key. Empty → keyword-heuristic mock mode. |
| `LLM_BASE_URL` | `https://api.openai.com/v1` | Any OpenAI-compatible endpoint. |
| `LLM_MODEL` | `gpt-4o-mini` | Model name for your provider. |
| `BLOCK_THRESHOLD` | `80` | Score ≥ this → `block` decision. |
| `ALERT_THRESHOLD` | `50` | Score ≥ this → `alert_analyst` decision. |
...
- Cold-start: new entities have no history — the system cannot distinguish benign recurring hosts from genuine first-time attackers until ≥ 20 events accumulate.
- Memory depth threshold: below ~18 entity events, the history_score contribution can slightly increase composite score even with FP discounts applied, marginally worsening FPR vs cold start (see benchmark Table 5).
- High-anomaly FP: events with anomaly > 0.65 remain above the alert threshold even with full benign memory — they correctly escalate to human review.
